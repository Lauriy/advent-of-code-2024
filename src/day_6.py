...
def has_cycle_with_steps(
    grid: list[list[str]],
    start_state: tuple[tuple[int, int], Direction],
    max_steps: int = 1000,
) -> bool:
    visited = {}
    ...
    for step in range(max_steps):
        if current_state in visited:
            ...
            return cycle_length <= max_steps // 2
...
def check_position_creates_loop(
    args: tuple[list[list[str]], tuple[int, int], tuple[int, int], Direction, bool],
) -> bool:
    ...
    while True:
        ...
        if not is_inside_grid(next_pos, local_grid):
            logger.debug(
                f"{thread_name}: Path escapes grid before hitting obstacle at ({x}, {y})",
            )
    ...
    step = 0
    max_steps = (
        len(local_grid) * len(local_grid[0]) * 4
    )  # width x height x 4 directions
    ...
    while step < max_steps:
        ...
        if not is_inside_grid(next_pos, local_grid):
            logger.debug(
                f"{thread_name}: Path escapes grid after {step} steps at ({x}, {y})",
            )
        ...
        if next_state in visited:
            ...
            if cycle_length <= max_steps // 2:
                if debug:
                    logger.debug(
                        f"Found loop at ({x}, {y}) affecting state at {curr_state.pos} facing {curr_state.direction}",
                    )
                logger.debug(
                    f"{thread_name}: Found cycle of length {cycle_length} at ({x}, {y})",
                )
    ...
    logger.debug(
        f"{thread_name}: No cycle found within {max_steps} steps at ({x}, {y})",
    )
...
def solve_first(input_path: str) -> int:
    ...
    while True:
        ...
        if not is_inside_grid(next_pos, grid) or grid[next_pos[1]][next_pos[0]] == "#":
            direction = direction.turn_right()
            continue
...
def solve_second(input_path: str) -> int:
    ...
    logger.debug(
        f"Checking {len(positions)} potential obstacle positions using {num_cores} CPU cores",
    )
    ...
    logger.debug(f"Found {total_loops} positions that create loops")
